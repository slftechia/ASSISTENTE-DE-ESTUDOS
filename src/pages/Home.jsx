...
export default function Home() {
  ...
  // Função para assinar plano ATUALIZADA
  const handleAssinarPlano = async (plano) => {
    // Se for o plano Free, a lógica continua a mesma.
    if (plano.nome === 'Free') {
      navigate('/register');
      return;
    }
    ...
    if (!user) {
      alert('Por favor, faça o login ou crie uma conta para poder assinar um plano.');
      // Opcional: pode redirecionar para a página de login/registro
      // navigate('/login'); 
      return;
    }

    try {
      // O AuthContext já injeta o token em todas as chamadas do axios.
      // Não é necessário adicionar o header de autorização manualmente aqui.
      const response = await axios.post(
        'http://localhost:5001/api/pagamento/criar-assinatura',
        plano
      );
      ...
      if (response.data.url) {
        ...
        alert('Erro ao iniciar o processo de assinatura. Tente novamente.');
      }
    } catch (error) {
      console.error('Erro ao criar assinatura:', error);
      const errorMsg = error.response?.data?.details || 'Erro ao conectar com o servidor de pagamento.';
      alert(errorMsg);
    }
  };
