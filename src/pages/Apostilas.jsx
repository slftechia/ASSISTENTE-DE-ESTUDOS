...
const Apostilas = () => {
  ...
  const buscarPDFsTema = async (tema) => {
    ...
    try {
      ...
      } else if (tema === 'especifica') {
        // Busca local dos PDFs específicos
        const cargoKey = cargoParaPDF[cargo || (user && user.cargo)] || cargo || (user && user.cargo);
        if (cargoKey) {
          setPdfsTema([
            {
              title: `Apostila Específica - ${formatarNomeCargo(cargoKey)}`,
              path: `/pdfs/apostilas_especificas/${cargoKey}.pdf`
            }
          ]);
        } else {
          setPdfsTema([]);
          ...
          <div>
            {!temaSelecionado ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                {(cargo || (user && user.cargo)) && (
                  <button
                    ...
                    onClick={() => buscarPDFsTema('especifica')}
                  >
                    Apostila Específica
                  </button>
                )}
              ...
              <div>
                ...
                <h2 className="text-2xl font-bold mb-6 text-center">{temas.find(t => t.valor === temaSelecionado)?.nome || (temaSelecionado === 'especifica' ? 'Apostila Específica' : '')}</h2>
                {loadingTema ? (
